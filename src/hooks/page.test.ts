...
describe("find_element_and_remove", () => {
  it("finds an element in a page", () => {
    ...
    expect(new_page.data).toEqual([{ id: "2", type: "div" }]);
  });
  ...
  it("finds in a complex tree", () => {
    ...
    expect(new_page.data[0].id).toEqual("1");
    expect(new_page.data[0].children?.[0].id).toEqual("3");
    expect(new_page.data[0].children?.[1].id).toEqual("5");
  });
});

describe("insert_element_at_idx", () => {
  it("insert at root in an empty page", () => {
    const page: Page = { data: [], title: "test" };
    const element = { id: "1", type: "div" };
    const idx = 0;
    const new_page = insert_element_at_idx(page, element, "root", idx);
    expect(new_page.data).toEqual([element]);
  });
  ...
  it("insert at root in a page with one element, two and three elements", () => {
    ...
    const page: Page = { data: [element1], title: "test" };
    const idx = 0;
    const new_page = insert_element_at_idx(page, element2, "root", idx);
    expect(new_page.data).toEqual([element2, element1]);

    const new_page2 = insert_element_at_idx(new_page, element3, "root", 2);
    expect(new_page2.data).toEqual([element2, element1, element3]);

    const new_page3 = insert_element_at_idx(new_page2, element4, "root", 1);
    expect(new_page3.data).toEqual([element2, element4, element1, element3]);
  });
  ...
  it("inserts elements in a first level child", () => {
    ...
    expect(new_page.data[0].id).toEqual("1");
    expect(new_page.data[0].children?.[0].id).toEqual("2");

    const new_page2 = insert_element_at_idx(new_page, element3, "1", 1);
    expect(new_page2.data[0].id).toEqual("1");
    expect(new_page2.data[0].children?.[0].id).toEqual("2");
    expect(new_page2.data[0].children?.[1].id).toEqual("3");

    const new_page3 = insert_element_at_idx(new_page2, element4, "1", 0);
    expect(new_page3.data[0].id).toEqual("1");
    expect(new_page3.data[0].children?.[0].id).toEqual("4");
    expect(new_page3.data[0].children?.[1].id).toEqual("2");
    expect(new_page3.data[0].children?.[2].id).toEqual("3");
  });
  ...
  it("inserts elements in a second level child, with a complex tree", () => {
    ...
    expect(new_page.data[0].id).toEqual("1");
    expect(new_page.data[0].children?.[0].id).toEqual("2");
    expect(new_page.data[0].children?.[1].id).toEqual("6");
    expect(new_page.data[0].children?.[2].id).toEqual("3");
    expect(new_page.data[0].children?.[2].children?.[0].id).toEqual("4");
    expect(new_page.data[0].children?.[3].id).toEqual("5");
  });
});
...
describe("move elements in the tree", () => {
  it("moves an element to the root", () => {
    ...
    expect(new_page.data[0].id).toEqual("2");
    expect(new_page.data[1].id).toEqual("1");
    expect(new_page.data[1].children?.[0].id).toEqual("3");
    expect(new_page.data[1].children?.[1].id).toEqual("5");
  });
  ...
  it("moves an element to a child", () => {
    ...
    expect(new_page.data[0].id).toEqual("1");
    expect(new_page.data[0].children?.[0].id).toEqual("3");
    expect(new_page.data[0].children?.[1].id).toEqual("2");
  });
  ...
  it("moves a deep element to the root", () => {
    ...
    expect(new_page.data[0].id).toEqual("4");
    expect(new_page.data[1].id).toEqual("1");
    expect(new_page.data[1].children?.[0].id).toEqual("2");
    expect(new_page.data[1].children?.[1].id).toEqual("3");
    expect(new_page.data[1].children?.[2].id).toEqual("5");
  });
});
